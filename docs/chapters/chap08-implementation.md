...
### 8.5.2 AD との整合性
「離散化してから微分（Discretize-then-Optimize）」のアプローチを取る限り、AD が計算するのは常に **ユークリッド内積に関する共ベクトル成分** である。
したがって、物理的に正しい「勾配」を得るには、必ず **適切な計量行列 $M^{-1}$ を手動で掛ける** 手順が必要になる。これを忘れると、メッシュ依存性のある（収束しない）コードになる。
