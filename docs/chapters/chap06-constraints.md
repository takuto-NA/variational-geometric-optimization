...
> Remark（KKT も「非線形作用素方程式」として扱う）
> KKT 条件は、拡張変数 $(x,\lambda,\mu)$ に対する残差 $R(x,\lambda,\mu)=0$ という非線形作用素方程式としてまとめられる。
> 実装ではこの残差（とその線形化）を matvec として扱い、サドル点ソルバへ接続する（第8章）。
...
> Remark（射影が「計量の話」になる理由）
> 「直交」や「射影」は内積（計量）があって初めて定義できる。したがって制約付きで“射影勾配”を使うことは、Geometry knob として $G$ を選んだことに相当する。
> 関数解析の最小背景（随伴・サドル点の安定性）は [付録C](./appC-functional-analysis-minimum) の C.3 と C.8 を参照。
