...
> Remark（方程式の見方）
> 第2章の停留条件 $d\mathcal F(x)=0$ は非線形作用素方程式（残差 $A(x)=0$）として読める。
> 本章の統一方程式は、その残差を $K/J$ で“動き”に変換して解くための進化方程式（流れ）である。
...
#### 記号衝突注意：散逸作用素 $K$ と FEM の剛性行列 $K$

本書で用いる散逸作用素 $K$ は、双対空間（共ベクトル）から接空間（ベクトル）への写像である。
一方で、有限要素法（FEM）などの分野では、剛性行列（Stiffness matrix）を慣習的に $K$ と書く。

- VGO の $K$: 共ベクトルを速度に変換する作用素（$T^* \to T$）。計量の逆（前処理／ソルバの適用）に相当。
- FEM の $K$: 変位を力に変換する作用素（$T \to T^*$）。二次変分（Hessian）$H$ に相当。

したがって、$\dot x = -K d\mathcal F$ という式を見たときに、FEM の文脈が染み付いていると「剛性行列を掛けて平滑化している」と誤読するリスクがあるが、実際には「剛性行列の逆（ソルバ）を掛けている」のが正解である。
（記号全体の最小表は [序文 0.5](./chap00-preface#0-5-記法最小) を参照。）
...
> Remark（同一視は「標準計量を選んだ」こと）
> ユークリッド空間では標準内積により $\mathbb R^n \simeq (\mathbb R^n)^*$ を同一視できるため、$d\mathcal F$ をベクトル $\nabla\mathcal F$ と書ける。
> これは「計量 $G=I$ を選んだ」と読むのが安全であり、一般の空間では計量（Riesz 同一視）が必要になる（背景は [付録C](./appC-functional-analysis-minimum) の C.6）。
