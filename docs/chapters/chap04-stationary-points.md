...
本書では「最適化」を広義に捉えるため、極小だけでなくサドルや退化を重要な対象として扱う。
...
- $\mathcal F$ を確実に減らすのは $K$ の散逸項だけである。
...
軌道は角速度 $k\omega$ で回転しながら、率 $k$ で減衰する。

## 4.6 この章のまとめ

本章では、停留点近傍の局所挙動を線形化ダイナミクス $A = (-K+J)H$ の固有値構造として読み解いた。
