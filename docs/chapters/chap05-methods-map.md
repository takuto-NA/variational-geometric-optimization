...
「最適化（極小化）」は停留点のうち安定なもの（極小点）を狙う立場だが、本書では広義に
...
ここで「幾何」の役割は次の通り：$G$ は距離（内積）を決め、$K = G^{-1}$ がどの方向を 1 とみなすか（前処理・自然勾配）に対応する。
$J$ は回転（保存）を作り、等高線に沿う成分を足す役割を持つ。

> Remark（$G$ は「問題ごとに選ぶ」）
> $G$ はユークリッド内積に限らず、関数空間では $L^2/H^1$、統計多様体では Fisher 計量などが代表例になる。
> 「計量が違うと勾配が変わる」背景は [付録C](./appC-functional-analysis-minimum) を参照。
...
「極小化」と同様の直観だけでは扱えない。
一方で第6章のようにラグランジアン $\mathcal L$ の停留構造として統一できる。

制約付きでは「目的関数を下げる」よりも、停留条件（KKT）を満たす点を探す方が自然になる。
