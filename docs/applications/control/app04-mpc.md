...
現場で direct が圧倒的に優勢な理由はシンプルで、

- 不等式制約（箱制約・ポリトープ・安全制約・接触など）を **KKT（相補性）として持ち込める**
- スパース構造（時系列の帯構造）を使って **毎ステップ高速に解ける**
- warm start / RTI により「完全収束」ではなく「追従」として回せる

からである。indirect（PMP/BVP）はここが難所になりやすい。
...
### 不等式・相補性・接触（実務で一番痛いところ）

MPC の「難しさの本体」は多くの場合、目的の微分よりも **不等式制約の扱い**にある。
特に接触・摩擦・スイッチングが入ると、active set（有効制約集合）が時々刻々変わり、数値的には次が問題になる：

- **相補性**: $\nu\ge 0, g\le 0, \nu^\top g=0$ が入ると、解は一般に非滑らか（モード切替）になる
- **解の不連続/不安定**: stick-slip や接触の on/off で線形化が急に変わる
- **ソルバの戦略**: active-set / interior-point / smoothing（緩和）などの選択が支配的になる

VGO 的には「制約付き停留（KKT）」の範囲で統一できる一方で、非滑らか性はこの地図の外縁に近づく（境界例は [第7章 7.2.1](../../chapters/chap07-cross-domain#7-2-1-境界例非滑らか最適化proxsubgradientはこの地図の外側)）。
...
- **実装リスク**:
  ...
  - 不等式/接触で active set が切り替わると不安定化しやすい（緩和、スムージング、ヒステリシス、モード設計などが要る）
