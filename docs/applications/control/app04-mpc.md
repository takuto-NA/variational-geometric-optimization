...
の KKT（Chapter 6）を毎サンプルで解く、という構造になる。
...
Chapter 5 の “手法マップ” に沿って **Newton 系を時間発展に埋め込んだ**と見なせる。
...
が取得できる（Chapter 8 の実装方針）。

## Notes

- **重要度**: 制約付き最適化（Chapter 6）と実装（Chapter 8）を「制御の現場」に接続する最短ルート。
